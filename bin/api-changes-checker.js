...
Promise.all(
                ...
                https
                    .get(url, resp => {
                        ...
                        resp.on("end", () => {
                            ...
                            saveFile(config.tmpPath + md5(url) + "_" + tempFile, data, function(err) {
                                if (err) {
                                    resolve({
                                        error: "NetworkError"
                                    })
                                ...
                                console.log("", "Done")
                                if (!fs.existsSync(config.tmpPath + md5(url) + "_" + baseFile)) {
                                    console.log("", "Local data does not exist, make it")
                                    saveFile(config.tmpPath + md5(url) + "_" + baseFile, data, function(err) {
                                        if (err) {
                                            return console.log(err)
                                        }
                    ...
                    .on("error", err => {
                        ...
                        printMessage(['Can\`t resolve remote data', url, 'API check bypassed'], {
                            borderColor: 'red'
                        })
                        process.exit()
    ...
    .then(results => {
        ...
        if (hasDiff) {
            printMessage(['Please, confirm that you are read this message, and we will proceed'], {
                borderColor: 'red'
            })
            rl.question(
                "[y/n] ",
...
function updateBaseFile(url) {
    const tmpData = fs.readFileSync(config.tmpPath + md5(url) + "_" + tempFile, "utf8")
